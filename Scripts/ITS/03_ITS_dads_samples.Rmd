...
filtered_read_counts<-read.csv("Data/ITS/filtered_data/ITS_subset_track_reads_dads.csv")
...
ggsave("Figures/ITS/relabundance_plot_treatment_relabund_genus_dads.pdf", abund_plot, width = 22, height = 16)
```
...
ggsave("Figures/ITS/relabundance_plot_soil_relabund_genus_dads.pdf", abund_plot, width = 22, height = 16)
```
...
ggsave("Figures/ITS/relabundance_plot_incubation_relabund_genus_dads.pdf", abund_plot, width = 22, height = 16)
```
...
pdf("Figures/ITS/top40_species_dads.pdf", width = 34, height = 20)
...
write.csv(data_summed, file="Results/ITS/mostabundanttaxa_abundance_dads_species.csv",row.names = TRUE)
...
write.csv(data_summed, file="Results/ITS/mostabundanttaxa_abundance_dads_genus.csv",row.names = TRUE)
...
write.csv(data_summed1, file="Results/ITS/mostabundanttaxa_relativeabundance_untreatedtreated_dads.csv",row.names = TRUE)
...
write.csv(data_summed2, file="Results/ITS/mostabundanttaxa_untreatedtreated_dads.csv",row.names = TRUE)
```
### Which samples of Stromatinia in them? 
```{r Identify samples with stromatinia}
tax_table <- as.data.frame(dads.ps@tax_table)

stromatinia_taxa <- apply(tax_table(dads.ps), 1, function(x) any(grepl("Stromatinia", x, ignore.case = TRUE)))
print(sum(stromatinia_taxa))

stromatinia_asvs <- rownames(tax_table(dads.ps))[stromatinia_taxa]

otu_table <- as.data.frame(as(otu_table(dads.ps), "matrix"))

if (ncol(otu_table) == nsamples(dads.ps)) {
  # If samples are in columns, transpose the table
  otu_table <- t(otu_table)
}

if ("ASV784" %in% colnames(otu_table)) {
  stromatinia_counts <- otu_table[, "ASV784", drop = FALSE]
  
  # Create a data frame with sample names and Stromatinia counts
  result <- data.frame(
    Sample = rownames(stromatinia_counts),
    Stromatinia_Count = stromatinia_counts[,1]
  )
  
  # Remove any samples with zero counts (if desired)
  result <- result[result$Stromatinia_Count > 0, ]
  
  # Print the dimensions and first few rows of the result
  print(dim(result))
  print(head(result))
} else {
  print("ASV784 not found in the OTU table.")
  
  # Print the first few column names of the OTU table for verification
  print("First few ASVs in the OTU table:")
...
write.csv(final_anova_table, "Results/ITS/final_anova_table_dads.csv", row.names = FALSE)
```
...
ggsave(plots_arrange_with_text, path = "Figures/ITS", filename = "alpha_diversity_each_factor_dads.pdf", 
       width = 12, height = 16)
```
...
# Save results
write.csv(final_results, file = "Results/ITS/PERMANOVA_interactions_with_betadisper.csv", row.names = FALSE)
```
...
ggsave("Figures/ITS/permanova_interactions_heatmap_dads.pdf", plot = heatmap, width = 8, height = 6)
```
...
# Export to CSV
write.csv(dads.wilcox.dt, file = "Results/ITS/wilcox_treat_vs_untreat_dads.csv", row.names = FALSE)
```
...
write.csv(dads.sig.dt, file="Results/ITS/deseq_trt_dads.csv",row.names = TRUE)
...
# Save the plot
ggsave("Figures/ITS/Deseq_treatment_barplot_dads_ASV.pdf", deseqplot2, width=40, height=40)
...
write.csv(final_table, "Results/ITS/merged_deseq_trt_wilcoxon_results.csv", row.names = FALSE)
...
# Save the plot
ggsave("Figures/ITS/Deseq_merged_trt_barplot_dads_ASV.pdf", merged_plot, height=30, width=40)
...
# Export to CSV
write.csv(dads.wilcox.dt, file = "Results/ITS/wilcox_soil_dads.csv", row.names = FALSE)
```
...
write.csv(dads.sig.dt, file="Results/ITS/deseq_soil_dads.csv",row.names = TRUE)
# Now, let's create the plot
...
# Save the plot
ggsave("Figures/ITS/Deseq_soil_barplot_dads_ASV.pdf", deseqplot2)
...
# Save the final table as CSV
write.csv(final_table, "Results/ITS/merged_deseq_soil_wilcoxon_results.csv", row.names = FALSE)
print(paste("Number of ASVs in the final table:", nrow(final_table)))
```
...
# Save the plot
ggsave("Figures/ITS/Deseq_merged_soil_barplot_dads_ASV.pdf", merged_plot)
...
# Export to CSV
write.csv(dads.wilcox.dt, file = "Results/ITS/wilcox_incubation_dads.csv", row.names = FALSE)
```
...
#mutate(across(everything(), ~ifelse(is.na(.), "Unclassified", .)))
# 
#write.csv(dads.sig.dt, # #file="Results/ITS/deseq_incubation_dads.csv",row.names = TRUE)
# 
# # Now, let's create the plot
# deseqplot2 <- ggplot(dads.sig.dt, aes(y = reorder(ASV, log2FoldChange), x = log2FoldChange, fill = Genus)) +
#   geom_vline(xintercept = 0.0, color = "gray", size = 0.5, linetype = "dashed") +
...
# 
# # Save the plot
# ggsave("Figures/ITS/Deseq_incubation_barplot_dads_ASV.pdf", deseqplot2)
# 
# # Display the plot
# print(deseqplot2)
```
...
# 
# # Save the final table as CSV
# write.csv(final_table, "Results/ITS/merged_deseq_incubation_wilcoxon_results.csv", row.names = FALSE)
# print(paste("Number of ASVs in the final table:", nrow(final_table)))
```
...
# 
# # Save the plot
# ggsave("Figures/ITS/Deseq_merged_incubation_barplot_dads_ASV.pdf", merged_plot)
# 
# # Display the plot
# print(merged_plot)
```
...
obj_ITS %>%
  ...
  heat_tree_matrix(data = "diff_table",
                   ...
                   key_size = 0.67,
                   output_file = "Figures/ITS/diff_abundance_heattree_Wilcoxonranksum_dads_ITS_fdr.pdf")
```
...
obj_ITS_rarefy %>%
  ...
  heat_tree_matrix(data = "diff_table",
                   ...
                   layout = "da", initial_layout = "re",
                   key_size = 0.67,
                   output_file = "Figures/ITS/diff_abundance_heattree_Wilcoxonranksum__rarefy_dads_ITS_fdr.pdf")
```
...
obj_ITS %>%
  ...
  heat_tree_matrix(data = "diff_table",
                   ...
                   layout = "da", initial_layout = "re",
                   key_size = 0.67,
                   output_file = "Figures/ITS/diff_abundance_heattree_Wilcoxonranksum_relabundance_dads_ITS_fdr.pdf")
```
...
obj_ITS %>%
  ...
  heat_tree_matrix(data = "diff_table_deseq",
                   ...
                   layout = "da", initial_layout = "re",
                   key_size = 0.4,
                   output_file = "Figures/ITS/diff_abundance_heattree_deseq_dads_ITS.pdf") # The layout algorithm that initializes node locations
```
