...
filtered_read_counts<-read.csv("Data/16S/filtered_data/16S_subset_track_reads_dads.csv")
summary(filtered_read_counts)
```

### Rarefaction curve 
```{r rarefaction curve}
otu_data <- as(otu_table(dads.ps), "matrix")
...
ggsave("Figures/16S/relabundance_plot_treatment_relabund_species_dads.pdf", abund_plot, width = 22, height = 16)
```

### Bar plot agglomerated by genus-using filtered dataset and faceted by soil 
...
ggsave("Figures/16S/relabundance_plot_soil_relabund_genus_dads.pdf", abund_plot, width = 22, height = 16)
```

### Bar plot agglomerated by genus-using filtered dataset and faceted by incubation
...
ggsave("Figures/16S/relabundance_plot_incubation_relabund_genus_dads.pdf", abund_plot, width = 22, height = 16)
```
...
pdf("Figures/16S/top40_genus_dads.pdf", width = 34, height = 20)
...
write.csv(data_summed, file="Results/16S/mostabundanttaxa_abundance_dads_genus.csv",row.names = TRUE)
...
write.csv(data_summed, file="Results/16S/mostabundanttaxa_abundance_dads_species.csv",row.names = TRUE)
...
write.csv(data_summed1, file="Results/16S/mostabundanttaxa_relativeabundance_untreatedtreated_dads.csv",row.names = TRUE)
...
write.csv(data_summed2, file="Results/16S/mostabundanttaxa_untreatedtreated_dads.csv",row.names = TRUE)
```
...
write.csv(final_anova_table, "Results/16S/final_anova_table_dads.csv", row.names = FALSE)
```
...
ggsave(plots_arrange_with_text, path = "Figures/16S", filename = "alpha_diversity_each_factor_dads.pdf", 
       width = 12, height = 16)
```
...
# Save results
write.csv(final_results, file = "Results/16S/PERMANOVA_interactions_with_betadisper.csv", row.names = FALSE)
```
...
ggsave("Figures/16S/permanova_interactions_heatmap_dads.pdf", plot = heatmap, width = 8, height = 6)
```
...
write.csv(ps_PCoA[["points"]], file="Results/16S/16S_PCoA_coordinates_persample.csv",row.names = TRUE)
...
# Perform PCoA ordination with k = 3
...
# Export to CSV
write.csv(dads.wilcox.dt, file = "Results/16S/wilcox_treat_vs_untreat_dads.csv", row.names = FALSE)
```
...
write.csv(dads.sig.dt, file="Results/16S/deseq_trt_dads.csv",row.names = TRUE)
...
# Save the plot
ggsave("Figures/16S/Deseq_treatment_barplot_dads_ASV.pdf", deseqplot2, width=40, height=40)
...
write.csv(final_table, "Results/16S/merged_deseq_trt_wilcoxon_results.csv", row.names = FALSE)
...
# Save the plot
ggsave("Figures/16S/Deseq_merged_trt_barplot_dads_ASV.pdf", merged_plot, height=30, width=40)
...
# Export to CSV
write.csv(dads.wilcox.dt, file = "Results/16S/wilcox_soil_dads.csv", row.names = FALSE)
```
...
write.csv(dads.sig.dt, file="Results/16S/deseq_soil_dads.csv",row.names = TRUE)
# Now, let's create the plot
...
# Save the plot
ggsave("Figures/16S/Deseq_soil_barplot_dads_ASV.pdf", deseqplot2)
...
# Save the final table as CSV
write.csv(final_table, "Results/16S/merged_deseq_soil_wilcoxon_results.csv", row.names = FALSE)
print(paste("Number of ASVs in the final table:", nrow(final_table)))
```
...
# Save the plot
ggsave("Figures/16S/Deseq_merged_soil_barplot_dads_ASV.pdf", merged_plot)
...
# Export to CSV
write.csv(dads.wilcox.dt, file = "Results/16S/wilcox_incubation_dads.csv", row.names = FALSE)
```
...
write.csv(dads.sig.dt, file="Results/16S/deseq_incubation_dads.csv",row.names = TRUE)
...
# Save the plot
ggsave("Figures/16S/Deseq_incubation_barplot_dads_ASV.pdf", deseqplot2)
...
# Save the final table as CSV
write.csv(final_table, "Results/16S/merged_deseq_incubation_wilcoxon_results.csv", row.names = FALSE)
print(paste("Number of ASVs in the final table:", nrow(final_table)))
```
...
# Save the plot
ggsave("Figures/16S/Deseq_merged_incubation_barplot_dads_ASV.pdf", merged_plot)
...
range(obj_16S$data$diff_table_deseq$padj, finite = TRUE) 
...
obj_16S %>%
  ...
  heat_tree_matrix(data = "diff_table_deseq",
                   ...
                   key_size = 0.4,
                   #node_size_range = c(.01, .055),
                   #node_label_size_range = c(.012, .032),
                   output_file = "Figures/16S/diff_abundance_heattree_deseq_dads_16S.pdf") # The layout algorithm that initializes node locations
```
